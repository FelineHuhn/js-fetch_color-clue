...
export async function fetchNewColor() {
  ...
  console.log(randomColor.name.value, randomColor.name.closest_named_hex);

  const colorName = randomColor.name.value;
  const closestNamedHex = randomColor.name.closest_named_hex;
